...
class EditProfilePageWeb extends StatefulWidget {
  ...
  const EditProfilePageWeb({Key? key, required this.name, required this.mobileNo, required this.address, required this.token, required this.userImageString}) : super(key: key);

  @override
  _EditProfilePageWebState createState() => _EditProfilePageWebState();
}
  ...
  Widget userImgNetwork() {
    return ClipOval(
      child: Material(
        color: Colors.transparent,
        child: Ink.image(
          image: NetworkImage(Config().mediaAPI + widget.userImageString),
          fit: BoxFit.cover,
          width: 140,
          height: 140,
          child: InkWell(onTap: () => pickImage(),),
